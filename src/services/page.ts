import { LandingPage, PricingPage, TemplatesPage } from "@/types/pages/landing";
...
export async function getTemplatesPage(locale: string): Promise<TemplatesPage> {
  return (await getPage("templates", locale)) as TemplatesPage;
}

export async function getPage(
  name: string,
  locale: string,
): Promise<LandingPage | PricingPage | TemplatesPage> {
  try {
    if (locale === "zh-CN") {
      locale = "zh";
    }
