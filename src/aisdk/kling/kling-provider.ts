...
export interface KlingProvider {
  image(modelId: KlingImageModelId, settings?: KlingImageSettings): KlingImageModel;
  video(modelId: KlingVideoModelId, settings?: KlingVideoSettings): KlingVideoModel;
}

export function createKling(options: KlingProviderSettings = {}): KlingProvider {
  const loadAccessKey = () =>
    loadSetting({
