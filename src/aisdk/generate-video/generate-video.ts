...
export async function generateVideo({
  ...
  for (const result of results) {
    videos.push(...result.videos.map((video) => new DefaultGeneratedVideo({ video })));
...
class DefaultGenerateVideoResult implements GenerateVideoResult {
  ...
  constructor(options: { videos: Array<DefaultGeneratedVideo>; warnings: Array<VideoGenerationWarning> }) {
