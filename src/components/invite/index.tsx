  ...
  return (
    <div className="flex flex-wrap gap-6">
      <Card className="flex-1 p-6">
        <h2 className="text-sm text-gray-500 mb-4">{t("my_invites.invite_code")}</h2>
        {user && user.uuid && (
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <span className="text-3xl font-bold">{user.invite_code || "NOT SET"}</span>
              <Icon name="RiEditLine" className="text-primary text-xl cursor-pointer" onClick={() => setOpen(true)} />
            </div>
            {user.invite_code && (
              <CopyToClipboard text={`${process.env.NEXT_PUBLIC_WEB_URL}/i/${user?.invite_code}`} onCopy={() => toast.success("copied")}>
      ...
      <Card className="flex-1 p-6">
        <div className="flex justify-between items-end mb-8">
          <div>
            <h2 className="text-sm text-gray-500 mb-4">{t("my_invites.invite_balance")}</h2>
        ...
        <div className="grid grid-cols-4 gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">{t("my_invites.total_invite_count")}</p>
          </div>
          <div>
            <p className="text-2xl font-bold">{summary.total_paid}</p>
            <p className="text-sm text-gray-500">{t("my_invites.total_paid_count")}</p>
          </div>
          <div>
            <p className="text-2xl font-bold">${summary.total_reward / 100}</p>
            <p className="text-sm text-gray-500">{t("my_invites.total_award_amount")}</p>
          </div>
        </div>
      </Card>
    </div>
