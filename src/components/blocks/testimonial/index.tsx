...
import { Heart, MessageCircle, MoreHorizontal, Repeat2, Star } from "lucide-react";
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {section.items?.map((tweet, index) => (
            <AnimatedSection key={index} delay={index * 100}>
              <div className="bg-white rounded-2xl p-6 border border-gray-100 hover:border-gray-200 hover:shadow-lg transition-all duration-300 transform hover:scale-105">
                {/* Tweet Header */}
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <img src={tweet.icon} alt={tweet.title} className="w-12 h-12 rounded-full object-cover" />
                    <div>
                      <div className="flex items-center space-x-1">
                        <span className="font-semibold text-gray-900">{tweet.title}</span>
                        {tweet.verified && (
                ...
                {/* Tweet Content */}
                <p className="text-gray-900 mb-4 leading-relaxed">{tweet.description}</p>
