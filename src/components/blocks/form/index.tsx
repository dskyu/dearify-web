...
export default function ({
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="w-full md:w-1/2 lg:w-1/2 space-y-6 px-2 pb-8">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    {item.title}
                    {item.validation?.required && <span className="text-red-500 ml-1">*</span>}
                  </FormLabel>
                  <FormControl>
                    {item.type === "textarea" ? (
                      <Textarea {...field} placeholder={item.placeholder} {...item.attributes} />
                    ) : item.type === "select" ? (
                      <Select onValueChange={field.onChange} defaultValue={field.value} {...item.attributes}>
                    ...
                    ) : item.type === "editor" || item.type === "richtext_editor" ? (
                      <Editor value={field.value} onChange={field.onChange} />
                    ) : (
                      <Input {...field} type={item.type || "text"} placeholder={item.placeholder} className="bg-background rounded-md" {...item.attributes} />
                    )}
                  </FormControl>
        ...
        {submit?.button && (
          <Button
            ...
            disabled={loading}
          >
            {loading ? <Loader className="mr-2 h-4 w-4 animate-spin" /> : submit.button.icon && <Icon name={submit.button.icon} className="size-4" />}
            {submit.button.title}
          </Button>
        )}
      </form>
