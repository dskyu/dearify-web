      ...
      {dataCards.map((dataCard, index) => (
        <Card className="@container/card" key={index}>
          <CardHeader>
            ...
            <CardTitle className="text-2xl font-semibold tabular-nums @[250px]/card:text-3xl">{dataCard.value}</CardTitle>
            <CardAction>{dataCard.label && <Badge variant="outline">{dataCard.label}</Badge>}</CardAction>
          </CardHeader>
          <CardFooter className="flex-col items-start gap-1.5 text-sm">
            <div className="line-clamp-1 flex gap-2 font-medium">{dataCard.description}</div>
